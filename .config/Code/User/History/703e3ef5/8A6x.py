...
class ModelSettings(BaseModel):
    ...
    chat_seed: Optional[int] = Field(
        default=None,
        description="Seed to make chat generations deterministic when supported",
    )
    chat_system_prompt: Optional[str] = Field(
        default=None,
        description="System prompt to prepend before user messages",
    )
    ...
    embed_batch_size: int = Field(
        default=128,
        description="Number of inputs batched per embedding call",
    )

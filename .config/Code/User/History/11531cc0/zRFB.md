...
```
5. Rebase from origin master to keep it updated:
```bash
git fetch origin
git rebase origin/master # resolve conflicts locally if needed
```
6. Push after rebase from master
```bash
git push --force-with-lease # while being on feature branch
```
7. Merge
```bash
...
8. (OPTIONAL) If you want to continue working inside this branch after merging and didn't delete it
```bash
